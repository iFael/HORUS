"""ETL da ANVISA — Agência Nacional de Vigilância Sanitária."""
...
class ANVISAETL(BaseETL):
    """Extrator de dados da ANVISA (dados abertos)."""

    nome_fonte = "anvisa"

    BASE_URL = "https://dados.anvisa.gov.br/dados/api/3/action"

    DATASETS = {
        "medicamentos": "medicamentos-registrados-e-com-precos",
        "saneantes": "saneantes",
    ...
    def _get_dataset(self, dataset_id: str) -> dict:
        rate_limiter.wait("anvisa", max_per_minute=15)
    ...
    def extract(self, **kwargs: Any) -> dict[str, dict]:
        ...
        for nome in datasets:
            ...
            try:
                ...
                self.logger.warning("Erro ANVISA %s: %s", nome, e)
        return result
    ...
    def load(self, df: pd.DataFrame, **kwargs: Any) -> int:
        if df.empty:
            return 0
        dest = self.config.paths.processed / "anvisa_dados.csv"
