"""ETL da ANCINE — Agência Nacional do Cinema."""
...
class ANCINEETL(BaseETL):
    """Extrator de dados da ANCINE (dados abertos)."""
    ...
    DATASETS = {
        "obras_audiovisuais": "obras-audiovisuais-brasileiras",
        "salas_exibicao": "salas-de-exibicao",
    }

    def _get_dataset(self, dataset_id: str) -> dict:
        rate_limiter.wait("ancine", max_per_minute=15)
    ...
    def extract(self, **kwargs: Any) -> dict[str, dict]:
        ...
        for nome in datasets:
            ...
            try:
                ...
                self.logger.warning("Erro ANCINE %s: %s", nome, e)
        return result
    ...
    def load(self, df: pd.DataFrame, **kwargs: Any) -> int:
        if df.empty:
            return 0
        dest = self.config.paths.processed / "ancine_dados.csv"
